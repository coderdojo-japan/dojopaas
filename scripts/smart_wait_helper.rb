...
module SmartWaitHelper
  ...
  # ディスクの準備を待つ
  def wait_for_disk_available(disk_id, options = {})
  ...
  # サーバーの起動/停止を待つ
  def wait_for_server_power(target_status, options = {})
    wait_for_resource("server", -> {
      power_status = get_server_power_status()
      current_status = power_status['Instance']['Status']
